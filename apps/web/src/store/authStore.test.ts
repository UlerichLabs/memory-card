...
describe('authStore', () => {
  ...
  it('logout bem-sucedido chama authService.logout e zera a sessão', async () => {
    const logoutSpy = vi.spyOn(authService, 'logout').mockResolvedValue(undefined)
    const { result } = await autenticado()
    expect(result.current.sessao).toEqual(sessao)
    await act(async () => result.current.logout())
    expect(logoutSpy).toHaveBeenCalledWith('access', 'refresh')
    expect(result.current.sessao).toBeNull()
  })
  it('logout com falha na chamada ao backend limpa o estado mesmo assim', async () => {
